...
export function VendorOnboardingForm() {
  ...
  const { register, handleSubmit, watch, setValue, trigger, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { shopName: '', description: '' },
  });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setLaunched(true);
    setTimeout(() => router.push('/vendor'), 2200);
  };

// Add useEffect to imports

// Add this state inside VendorOnboardingForm
...
// Replace the launched screen's confetti map with:
if (launched) {
  return (
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .ob-preview-card {
          ...
          border-radius:18px; padding:20px; margin-top:32px;
          transition: all 0.3s;
        }
        ...
        /* Two-col grid */
        .ob-grid2 { display:grid; grid-template-columns:1fr 1fr; gap:16px; }
        ...
        .ob-btn-back:hover { border-color:#d1d5db; background:#f9fafb; }
        .ob-btn-next {
          flex:1; display:flex; align-items:center; justify-content:center; gap:8px;
          padding:13px 24px; background:#111; border:none; border-radius:12px;
        ...
        .ob-btn-launch {
          flex:1; display:flex; align-items:center; justify-content:center; gap:8px;
          padding:13px 24px; background:linear-gradient(135deg,#16a34a,#15803d); border:none; border-radius:12px;
          font-size:14px; font-weight:700; color:#fff; cursor:pointer;
          transition:all 0.2s; font-family:'DM Sans',sans-serif;
          box-shadow: 0 4px 14px rgba(22,163,74,0.35);
        }
        .ob-btn-launch:hover { transform:translateY(-1px); box-shadow:0 6px 20px rgba(22,163,74,0.4); }
        .ob-btn-launch:disabled { opacity:0.6; cursor:not-allowed; transform:none; }
        ...
        <div className="ob-left">
          ...
          <div className="ob-preview-card">
            ...
            <div className="ob-preview-meta">
              ...
              {values.phone
                ? <span className="ob-preview-badge">📞 {values.phone}</span>
                : null}
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              {step === 1 && (
                <>
                  ...
                  <div className="ob-field">
                    <label className="ob-label"><ImageIcon size={13} style={{ display:'inline', marginRight:5 }} />Shop Logo</label>
                    ...
                    {values.logoUrl ? (
                      <div style={{ display:'flex', alignItems:'center', gap:14 }}>
                        ...
                        <div>
                          ...
                          <button type="button" onClick={() => setValue('logoUrl', null)} style={{ fontSize:12, color:'#9ca3af', background:'none', border:'none', cursor:'pointer', padding:0, marginTop:2 }}>Remove</button>
                        </div>
                      </div>
                    ) : (
                      <div className="ob-upload" onClick={() => logoInputRef.current?.click()}>
                        <div className="ob-upload-icon">🖼️</div>
                        <div className="ob-upload-text">Upload your shop logo</div>
                        <div className="ob-upload-sub">PNG, JPG or WebP · Max 2MB · Square recommended</div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="ob-field">
                    <label className="ob-label"><Phone size={13} style={{ display:'inline', marginRight:5 }} />Business Phone</label>
                    <input {...register('phone')} className="ob-input" placeholder="+254712345678" />
                  </div>

                  <div className="ob-field">
                    <label className="ob-label"><AlignLeft size={13} style={{ display:'inline', marginRight:5 }} />Shop Description</label>
                    <textarea {...register('description')} className="ob-input ob-textarea" placeholder="What do you sell? What makes your shop special? The more detail, the better." />
                  </div>
                </>
              )}
              ...
              {step === 2 && (
                <>
                  <div className="ob-step-header">
                    ...
                    <div className="ob-step-subtitle">Required for payouts above KES 70,000. You can skip and add later.</div>
                  </div>

                  <div className="ob-info">
                    💡 All information is encrypted and only used for tax compliance and payout verification.
                  </div>

                  <div className="ob-field">
                    <label className="ob-label"><Building2 size={13} style={{ display:'inline', marginRight:5 }} />Legal Business Name</label>
                    <input {...register('legalName')} className="ob-input" placeholder="Registered business name (if any)" />
                  </div>

                  <div className="ob-field">
                    <label className="ob-label"><Hash size={13} style={{ display:'inline', marginRight:5 }} />KRA PIN</label>
                    <input {...register('kraPin')} className="ob-input" placeholder="e.g. A000000000X" style={{ textTransform: 'uppercase', letterSpacing: '0.08em' }} />
                  </div>
                  ...
                  <div className="ob-field">
                    ...
                    <div className="ob-upload" onClick={() => docInputRef.current?.click()}>
                      ...
                      <div className="ob-upload-sub">PDF, JPG or PNG · Max 5MB</div>
              ...
              {step === 3 && (
                <>
                  ...
                  <div className="ob-field">
                    <label className="ob-label"><MapPinned size={13} style={{ display:'inline', marginRight:5 }} />City / Town</label>
                    <input {...register('city')} className="ob-input" placeholder="e.g. Nairobi, Mombasa, Kisumu, Nakuru..." />
                  </div>

                  <div className="ob-field">
                    <label className="ob-label"><MapPin size={13} style={{ display:'inline', marginRight:5 }} />Street / Building Address</label>
                    <input {...register('address')} className="ob-input" placeholder="e.g. Tom Mboya St, Jubilee Exchange, 3rd Floor" />
                  </div>

                  <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:12, padding:'14px 16px', fontSize:13, color:'#15803d', display:'flex', gap:10, alignItems:'flex-start' }}>
                    <span style={{ fontSize:16 }}>📦</span>
                    <span>Your address is never shown publicly — it's used for delivery coordination and verification only.</span>
                  </div>
                </>
              )}
              ...
              {step === 4 && (
                <>
                  ...
                  <div className="ob-review-section">
                    ...
                    {[
                      { k: 'Shop Name',    v: values.shopName },
                      { k: 'Phone',        v: values.phone },
                      { k: 'Description',  v: values.description },
                      { k: 'Logo',         v: values.logoUrl ? '✓ Uploaded' : '—' },
                    ].map(r => (
                      <div className="ob-review-row" key={r.k}>
                        <span className="ob-review-key">{r.k}</span>
                        <span className="ob-review-val" style={{ color: r.v ? '#111' : '#9ca3af' }}>
                          {r.v || '—'}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="ob-review-section">
                    <div className="ob-review-section-title">Legal</div>
                    {[
                      { k: 'Legal Name', v: values.legalName },
                      { k: 'KRA PIN',    v: values.kraPin },
                      { k: 'KRA Doc',    v: values.kraPinDoc ? '✓ Uploaded' : '—' },
                    ].map(r => (
                      <div className="ob-review-row" key={r.k}>
                        <span className="ob-review-key">{r.k}</span>
                        <span className="ob-review-val" style={{ color: r.v ? '#111' : '#9ca3af' }}>{r.v || '—'}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:12, padding:'14px 16px', fontSize:13, color:'#15803d', marginTop:16 }}>
                    ✅ Your shop goes live immediately. Our team will review your details within 24 hours and notify you.
                  </div>
                </>
              )}
              ...
              <div className="ob-nav">
                ...
                {step < 4 ? (
                  ...
                  <button type="submit" className="ob-btn-launch" disabled={isSubmitting}>
                    {isSubmitting ? <><Loader2 size={15} className="animate-spin" /> Setting up...</> : <>🚀 Launch My Shop</>}
                  </button>
                )}
              </div>
            </form>
