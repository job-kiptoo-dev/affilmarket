...
import { requestVendorPayoutAction }         from '@/action/vendorPayoutAction';
import type { VendorPayoutsPageData, PayoutMethod, PayoutStatus } from '@/action/vendorPayoutAction';
import {
  Wallet, Clock, ArrowUpRight, AlertCircle,
  CheckCircle, XCircle, Smartphone, Building2,
} from 'lucide-react';
...
export function VendorPayoutsClient({ data }: { data: VendorPayoutsPageData }) {
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await requestVendorPayoutAction(parsed, method);
      if (result.success) {
  ...
  return (
    <>
      <style>{`
        ...
        .po-radio { width: 16px; height: 16px; border-radius: 50%; border: 2px solid #d1d5db; margin-left: auto; flex-shrink: 0; position: relative; }
        ...
        .po-dest { background: #f9fafb; border: 1px solid #f3f4f6; border-radius: 10px; padding: 13px 16px; margin-bottom: 20px; }
        ...
        .po-input  { width: 100%; border: 1px solid #d1d5db; border-radius: 9px; padding: 12px 14px 12px 56px; font-size: 16px; font-weight: 700; color: #111; font-family: 'DM Sans', sans-serif; outline: none; transition: border-color 0.15s; box-sizing: border-box; }
        ...
        @media (max-width: 640px) { .po-grid { grid-template-columns: 1fr; } .po-method-row { flex-direction: column; } }
      `}</style>

      <div className="po">
        <h1 className="po-h1">Withdraw Earnings</h1>
        <p className="po-sub">Request a payout to your M-Pesa account</p>
        ...
        <div className="po-grid">
          ...
          <div className="po-bal">
            ...
            <div className="po-bal-hint">⏳ Released after delivery</div>
          </div>
        ...
        <div className="po-card">
          ...
          {!data.hasPendingRequest && data.availableBalance < 100 && (
            <div className="po-info">
              ...
              You need at least KES 100 available to withdraw.
            </div>
          )}
          {submitSuccess && (
            <div className="po-success"><CheckCircle size={15} /> Payout requested! We'll process it within 1–2 business days.</div>
          )}
          {submitError && (
            <div className="po-error"><XCircle size={15} /> {submitError}</div>
          )}
          ...
          <div className="po-method-row">
            {(['MPESA', 'BANK'] as PayoutMethod[]).map((m) => {
              ...
              const hint = isMpesa ? (data.mpesaPhone ?? 'No number on file') : 'Coming soon';
              return (
                <button
                  key={m}
                  className={`po-method-btn${method === m ? ' active' : ''}`}
                  onClick={() => setMethod(m)}
                  disabled={!canRequest || (!isMpesa)}
                >
                  <div style={{ width: 34, height: 34, borderRadius: 8, background: isMpesa ? '#dcfce7' : '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {isMpesa ? <Smartphone size={16} style={{ color: '#16a34a' }} /> : <Building2 size={16} style={{ color: '#2563eb' }} />}
                  </div>
                  <div>
          ...
          {canRequest && (
            <div className="po-quick">
              ...
              <button className="po-quick-btn" onClick={() => setAmount(String(Math.floor(data.availableBalance)))}>
                Max ({formatKES(data.availableBalance)})
              </button>
            </div>
          )}

          <button className="po-btn" onClick={handleSubmit} disabled={!canRequest || isPending || !amount}>
            {isPending
