...
import { Eye, EyeOff, ShoppingBag, Loader2, Store, Link2, Layers, AlertCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { registerUser } from '@/action/registerAction';

type RegisterForm = z.infer<typeof RegisterSchema>;

// Map server-returned error codes/messages to user-friendly text
const SERVER_ERROR_MAP: Record<string, { field?: keyof RegisterForm; message: string }> = {
  'Email already in use':         { field: 'email',    message: 'This email is already registered. Try signing in.' },
  'Phone already in use':         { field: 'phone',    message: 'This phone number is already linked to an account.' },
  'DUPLICATE_EMAIL':              { field: 'email',    message: 'This email is already registered. Try signing in.' },
  'DUPLICATE_PHONE':              { field: 'phone',    message: 'This phone number is already linked to an account.' },
  'INVALID_EMAIL':                { field: 'email',    message: 'Enter a valid email address.' },
  'WEAK_PASSWORD':                { field: 'password', message: 'Choose a stronger password.' },
};
...
const onSubmit = async (data: RegisterForm) => {
  ...
  if (!result.success) {
    if (result.field) {
      setError(result.field as keyof RegisterForm, {
        type: 'server',
  ...
  try {
    ...
    // session is valid, don't block navigation
  }

  router.push(result.role === 'AFFILIATE' ? '/affiliate' : '/vendor');
};
  return (
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={cn(
                    'w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 transition-all pr-10',
                    errors.password
                      ? 'border-red-400 focus:ring-red-200 focus:border-red-400'
                      : 'border-gray-200 focus:ring-brand-green/20 focus:border-brand-green'
                  )}
