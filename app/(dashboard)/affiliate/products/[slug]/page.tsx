...
import {
  products, vendorProfiles, categories, orders,
} from '@/drizzle/schema';
...
import { getAuthUser }     from '@/lib/healpers/auth-server'; // ✅ added
...
function normaliseRate(raw: string | number | null | undefined): number {
...
export default async function ProductPage({
  ...
  // ✅ Run both fetches in parallel — no extra waterfall
  ...
  const commRate = normaliseRate(product.affiliateCommissionRate);
  const commAmt  = (price * commRate).toFixed(0);
  const baseUrl  = process.env.NEXT_PUBLIC_APP_URL ?? '';
  const affLink  = `${baseUrl}/products/${slug}${aff ? `?aff=${aff}` : ''}`;
  const inStock  = parseInt(String(product.stockQuantity ?? '0'), 10) > 0;

  const gallery: string[] = [
    ...(product.mainImageUrl ? [product.mainImageUrl] : []),
    ...((product.galleryImages as string[] | null) ?? []),
  ];

  // ✅ Derive display name + initials from the session user
  const userName     = authUser?.name ?? authUser?.email?.split('@')[0] ?? null;
  const userInitials = userName ? userName.slice(0, 2).toUpperCase() : null;
  ...
  return (
    <>
      <style>{`
        ...
        .pp-nav-user { display: flex; align-items: center; gap: 9px; }
        .pp-nav-avatar {
        ...
        .pp-nav-name { font-size: 13px; font-weight: 600; color: #374151; }
        ...
        .pp-gallery { display: flex; gap: 8px; flex-wrap: wrap; }
        .pp-thumb { width: 68px; height: 68px; border-radius: 10px; overflow: hidden; border: 2px solid #e5e7eb; cursor: pointer; background: #f3f4f6; flex-shrink: 0; transition: border-color 0.15s; }
        .pp-thumb img { width: 100%; height: 100%; object-fit: cover; }
        .pp-thumb:hover { border-color: #16a34a; }
        ...
        .pp-stock-in  { font-size: 13px; color: #16a34a; font-weight: 600; margin-bottom: 16px; }
        .pp-stock-out { font-size: 13px; color: #ef4444; font-weight: 600; margin-bottom: 16px; }
        ...
        .pp-aff-badge { background: #111; border-radius: 10px; padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-top: 14px; }
        ...
        .pp-share-box { background: #111; border-radius: 14px; padding: 18px 20px; }
        .pp-share-title { font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #6b7280; margin-bottom: 10px; }
        .pp-share-url { font-family: monospace; font-size: 12px; color: #9ca3af; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 8px 12px; word-break: break-all; margin-bottom: 10px; }
        @media (max-width: 800px) { .pp-body { grid-template-columns: 1fr; } .pp-nav-name { display: none; } }
      `}</style>
      ...
      <div className="pp">
        ...
        <nav className="pp-nav">
          ...
          {/* ✅ Avatar + name when logged in, login button when not */}
          {authUser && userName ? (
            <div className="pp-nav-user">
              <div className="pp-nav-avatar">{userInitials}</div>
              <span className="pp-nav-name">{userName}</span>
            </div>
          ) : (
            <Link
              href="/login"
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
    <StarDisplay rating={parseFloat(product.avgRating ?? '0')} size={16} />
...
<ReviewsSection productId={product.id} />
              ...
              <div className={inStock ? 'pp-stock-in' : 'pp-stock-out'}>
                {inStock
                  ? `✓ ${parseInt(String(product.stockQuantity), 10)} in stock`
                  : '⚠ Out of stock'}
              </div>
...
/>
            ...
            {aff && (
              <div className="pp-comm-card">
                ...
                <div className="pp-comm-amount">+KES {commAmt}</div>
            ...
            {/* Share / affiliate link box */}
            <div className="pp-share-box">
              <div className="pp-share-title">
                {aff ? '🔗 Your affiliate link' : '🔗 Share this product'}
              </div>
              <div className="pp-share-url">{affLink}</div>
              <CopyLinkButton url={affLink} />
            </div>
            ...
            <div className="pp-card">
              ...
              <div className="pp-vendor-card">
                <div className="pp-vendor-logo">
                  {product.logoUrl
                    ? <img src={product.logoUrl} alt={product.shopName ?? 'Vendor'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : <Store size={20} color="#16a34a" />}
                </div>
                <div>
                  <div className="pp-vendor-name">{product.shopName ?? 'AffilMarket Vendor'}</div>
                  <div className="pp-vendor-sub">
                    {product.shopDescription
                      ? product.shopDescription.length > 60
                        ? product.shopDescription.slice(0, 60).trimEnd() + '…'
                        : product.shopDescription
                      : 'Verified vendor on AffilMarket'}
                  </div>
                </div>
              </div>
            </div>
